...
def train_model(model, training_data, test_data, test_data_pointcloud, training_params):
    ...
    with torch.no_grad():
        ...
        point_cloud_error = (model.forward_BB(test_input_local) - test_BB_local).norm(dim = -1)
    ...
    #plt.savefig('Figures_paper/Fig_training_2d_no_prior/training_loss')
    plt.show()
    
    plt.plot(Test_loss_history)
    plt.title('Test Loss History')
    #plt.savefig('Figures_paper/Fig_training_2d_no_prior/test_loss')
    plt.show()
    ...
    plt.ylim(0)
    #plt.savefig('Figures_paper/Fig_training_2d_no_prior/point_cloud_error')
    plt.show()